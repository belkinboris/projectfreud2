// ------- Global functions so the HTML inline onclick can see them ------- //
function goToDiary() {
  saveGameState();
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  console.log("happinessBar:", happinessBar);
  console.log("healthBar:", healthBar);
  console.log("smartsBar:", smartsBar);
  console.log("looksBar:", looksBar);
  ...
  function getRandomEvent() {
    const availableEvents = events.filter(
      (event) => age >= event.ageRange[0] && age <= event.ageRange[1]
    );
  ...
  function triggerRandomRelationshipEvent() {
    const availableEvents = relationshipEvents.filter(
      (event) => age >= event.ageRange[0] && age <= event.ageRange[1]
    );
    if (availableEvents.length > 0) {
      const event =
  ...
  function handleChoices(event) {
    ...
    updateEventLog(
      `Age ${age}: ${event.text} You chose '${
        userChoice ? "Yes" : "No"
      }'. (${event.stat} ${effect > 0 ? "+" : ""}${effect})`
    );
  }
  ...
  function updateStat(stat, change) {
    ...
    if (stat === "smarts") {
      smarts = Math.max(0, Math.min(100, smarts + change));
    }
    if (stat === "looks") {
      looks = Math.max(0, Math.min(100, looks + change));
    }
    updateStatsDisplay(); // Call to refresh the UI after stat update
  }

  // Overload the function name again to ensure both definitions exist
  function updateStatsDisplay() {
  ...
  function updateEventLog(message) {
    ...
    newEvent.textContent = message; // Set the event message
    eventLog.appendChild(newEvent); // Append the new event
  ...
  function getRandomEvent() {
    ...
    if (availableEvents.length === 0) {
      ...
      return null; // If no events match the current age
    }
